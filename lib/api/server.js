...
export async function serverFetchSearchAllContent(params) {
    const queryString = new URLSearchParams(params).toString();
    const url = `${API_BASE_URL}/search-service/v1.1/universal-search?${queryString}`;
    return serverFetch(url);
}

export async function serverFetchSearchMovieContent(params) {
    const queryString = new URLSearchParams(params).toString();
    const url = `${API_BASE_URL}/search-service/v1.1/universal-search?${queryString}`;
    return serverFetch(url);
}

export async function serverFetchSearchShowContent(params) {
    const queryString = new URLSearchParams(params).toString();
    const url = `${API_BASE_URL}/search-service/v1.1/universal-search?${queryString}`;
    return serverFetch(url);
}
