...
/**
 * ProviderFeaturedCarousel — Client Component (interactive only).
 * Receives `items` as a prop from the Server Component parent.
 */
const ProviderFeaturedCarousel = ({ items = [] }) => {
    ...
    const getImageUrl = (item) => {
        ...
        if (item.content_type === "movie" && item.movie?.backdrops?.[0])
            return item.movie.backdrops[0].url || item.movie.backdrops[0];
        if (item.content_type === "show" && item.show?.backdrops?.[0])
        ...
        <div className="w-full">
                                                ...
                                                <div className="relative w-full" style={{ aspectRatio: "16/9" }}>
                                                    {imageUrl ? (
                                                        ...
                                                        <div className="w-full h-full bg-linear-to-br from-gray-700 to-gray-900 flex items-center justify-center">
                                                            <span className="text-gray-400">{itemTitle}</span>
                                                        </div>
                                                    )}
            ...
            <div className="flex justify-center items-center gap-2 mt-4 flex-wrap min-h-4">
                {items.map((item, index) => (
                    <button
                        key={item.order || index}
                        onClick={() => { carouselApi?.scrollTo(index); setCurrentIndex(index); }}
